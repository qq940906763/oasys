...
public interface UserDao extends JpaRepository<User, Long>{
	 ...
	 * @return
	 */
	@Query("from User u where u.isLock=?1 and u.dept.sysId=?2")
	Page<User> findByIsLock(Integer isLock,String sysId,Pageable pa);
	
	
	@Query("from User u where (u.dept.deptName like %?1% or u.userName like %?1% or u.realName like %?1% or u.userTel like %?1% or u.role.roleName like %?1%) and u.dept.sysId=?2")
	Page<User> findnamelike(String name,String sysId,Pageable pa);
