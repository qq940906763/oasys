...
public class DeptController {
	...
	public String deptmanage(HttpServletRequest req, Model model) {
		...
		List<Dept> depts = deptdao.findDeptBySysId(user.getDept().getSysId());
	...
	public String adddept(HttpServletRequest req,@Valid Dept dept,@RequestParam("xg") String xg,BindingResult br,Model model){
		...
		if(!br.hasErrors()){
			...
			if("add".equals(xg)){
				...
				pdao.save(jinli);
				pdao.save(wenyuan);
				adddept.setSysId(user.getDept().getSysId());
				deptdao.save(adddept);
			}
			if(adddept!=null){
